...
mod my_module {
    ...
    pub fn transformer(input: Vec<(&str, Command)>) -> Vec<String> {
        ...
        for (string, command) in input.iter() {
            ...
            // let mut stg: String = string.clone().to_string();
            // let up_str: String = string.to_uppercase();
            match command {
                Command::Uppercase => output.push(string.to_uppercase()),
                Command::Trim => output.push(string.trim().to_string()),
                Command::Append(sz) => {
                    let mut stg = string.to_owned().to_string();
                    for _i in 0..*sz as u64 {
                        stg.push_str(&"bar".to_string())
                    }
                    output.push(stg)
